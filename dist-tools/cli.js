...
const str = (flags, key) => typeof flags[key] === "string" ? flags[key] : void 0;
const bool = (flags, key) => flags[key] === true || flags[key] === "true";
async function load(paths, flags) {
  ...
  for (const path of paths) {
    ...
    if (result.cues.length === 0) {
      throw new UserError(
        `No subtitle cues found in ${path}. Detected format: ${result.format}.` + (result.warnings.length > 0 ? `
...
`);
    ...
    const language = detected !== "und" ? detected : str(flags, "input-language") ?? str(flags, "language") ?? "en";
    evidence.push(
...
async function cmdBuild(args) {
  ...
  const format = str(args.flags, "format") ?? "fountain";
  ...
  const document = renderScreenplay(loaded.scenes, {
    language,
    ...str(args.flags, "secondary-language") ? { secondaryLanguage: str(args.flags, "secondary-language") } : {},
...
async function cmdSearch(args) {
  ...
  const results = searchScreenplay(loaded.scenes, query, {
    scope: str(args.flags, "scope") ?? "all",
